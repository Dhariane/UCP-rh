...
class CongePlanifieController(APIView):
    ...
    def get(self, request, personnel_id):
        try:
            conges = CongePlanifie.objects.select_related('type_conge').filter(personnel_id=personnel_id).order_by('-date_debut')
    ...
    def post(self, request, personnel_id):
        try:
            ...
            try:
                ...
                # ✅ Correction de la faute de frappe HTTP_44 -> HTTP_404
            ...
            # Déclenche la validation clean() du modèle
            conge_planifie.full_clean()
            conge_planifie.save()

            # ✅ Alignement parfait avec le Front : le composant vérifie result.status === 'success'
            return Response({
                "status": "success",
            ...
            # ✅ On extrait le premier message d'erreur sous forme de chaîne pour éviter de faire cracher le Toaster React
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. GESTION PAR CRÉNEAU UNIQUE (MODIFIER & SUPPRIMER)
# ─────────────────────────────────────────────────────────────────────────────
class CongePlanifieDetailController(APIView):

    # PATCH / PUT : Modifier un congé planifié existant
    def patch(self, request, id):
        try:
            conge_planifie = CongePlanifie.objects.get(id=id)
            data = request.data

            # Mise à jour optionnelle du type de congé
            type_conge_id = data.get('type_conge_id')
            if type_conge_id:
                try:
                    ...
                    # ✅ Correction de la faute de frappe HTTP_44 -> HTTP_404
                    return Response({"error": "Le type de congé spécifié n'existe pas"}, status=status.HTTP_44_NOT_FOUND)

            # Mise à jour des autres champs si fournis
            if 'date_debut' in data:
                conge_planifie.date_debut = data.get('date_debut')
            if 'date_fin' in data:
                conge_planifie.date_fin = data.get('date_fin')
            if 'description' in data:
                conge_planifie.description = data.get('description')

            # Validation des règles métiers et sauvegarde
            conge_planifie.full_clean()
            conge_planifie.save()
